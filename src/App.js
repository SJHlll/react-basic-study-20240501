...
const App = () => {
  ...
  // 리렌더링 될 때마다 실행하면 안된다 (한번만 확인)
  useEffect(() => {
    console.log('useEffect 실행 - 최초 1회');
  ...
  // 서버로 로그인을 요청하는 함수 (나중에 fetch를 통해 백엔드와의 연계 필요)
  const loginHandler = (email, password) => {
    // 로그인을 했다는 증거로 상태값 변경 및 브라우저에 로그인 값을 1로 표현해서 저장
    // (이전 프로젝트에서는 로그인 유지를 session을 사용함)
    localStorage.setItem('login-flag', '1'); // 브라우저에서 제공하는 저장소
    setIsLoggedIn(true);
  };
