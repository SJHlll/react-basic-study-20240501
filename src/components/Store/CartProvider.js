...
const cartReducer = (state, action) => {
  ...
  if (action.type === 'ADD') {
    ...
    // findIndex: 콜백을 이용해 배열을 순회하면서 지정한 조건에 맞는 요소의 인덱스를 반환
    const index = state.items.findIndex(
      // 기존 상태 배열의 id를 하나식 얻어 현재 추가하고자 하는 상품의 id와 같은 요소의 인덱스 반환
    ...
    if (index === -1) {
      ...
      // 이미 추가된 아이템 (수량 +1)
      prevCartItem.amount += newCartItem.amount; // ++은 안됨, 바깥 화면에서는 상품이 하나씩 올라가는 것은 아님
    ...
    // 업데이트 전의 수량이 1이면 filter를 이용해 카트에서 아예 빼버린다
    // 1보다 크면 filter로 제거하지 않고, 기존 배열에 수량만 1 내린 채로 업데이트한다
    let removedItems;
